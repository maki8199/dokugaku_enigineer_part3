...
function getViewingTimePerChannel(array $inputs): array
{
    ...
    foreach ($inputs as $input) {
        ...
        $min = (int)$input[1];
        $mins = [$min];
        if (array_key_exists($ch, $viewingTimePerChannel)) {
            $mins = array_merge($viewingTimePerChannel[$ch], $mins);
        }
        $viewingTimePerChannel[$ch] = $mins;
    }
...
function display(array $viewingTimePerChannel, float $totalViewingTime): void
{
    ...
    foreach ($viewingTimePerChannel as $ch => $mins) {
